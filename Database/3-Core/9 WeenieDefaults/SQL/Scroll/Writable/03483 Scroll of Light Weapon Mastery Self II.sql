DELETE FROM `weenie` WHERE `class_Id` = 3483;

INSERT INTO `weenie` (`class_Id`, `class_Name`, `type`, `last_Modified`)
VALUES (3483, 'scrollspearmasteryself2', 34, '2019-02-10 00:00:00') /* Scroll */;

INSERT INTO `weenie_properties_int` (`object_Id`, `type`, `value`)
VALUES (3483,   1,       8192) /* ItemType - Writable */
     , (3483,   5,         30) /* EncumbranceVal */
     , (3483,  16,          8) /* ItemUseable - Contained */
     , (3483,  19,          5) /* Value */
     , (3483,  93,       1044) /* PhysicsState - Ethereal, IgnoreCollisions, Gravity */
     , (3483, 8041,        101) /* PCAPRecordedPlacement - Resting */;

INSERT INTO `weenie_properties_bool` (`object_Id`, `type`, `value`)
VALUES (3483,  22, True ) /* Inscribable */;

INSERT INTO `weenie_properties_float` (`object_Id`, `type`, `value`)
VALUES (3483,  39,     1.5) /* DefaultScale */;

INSERT INTO `weenie_properties_string` (`object_Id`, `type`, `value`)
VALUES (3483,   1, 'Scroll of Light Weapon Mastery Self II') /* Name */;

INSERT INTO `weenie_properties_d_i_d` (`object_Id`, `type`, `value`)
VALUES (3483,   1, 0x0200018A) /* Setup */
     , (3483,   8, 0x06007119) /* Icon */
     , (3483,  22, 0x3400002B) /* PhysicsEffectTable */
     , (3483,  28,        299) /* Spell - LightWeaponsMasterySelf2 */
     , (3483, 8001,    6307864) /* PCAPRecordedWeenieHeader - Value, Usable, Container, Burden, Spell */
     , (3483, 8003,         18) /* PCAPRecordedObjectDesc - Inscribable, Attackable */
     , (3483, 8005,     135297) /* PCAPRecordedPhysicsDesc - CSetup, ObjScale, PeTable, AnimationFrame */;

INSERT INTO `weenie_properties_i_i_d` (`object_Id`, `type`, `value`)
VALUES (3483, 8000, 0x858EC09E) /* PCAPRecordedObjectIID */;
