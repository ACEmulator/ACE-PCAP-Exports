DELETE FROM `weenie` WHERE `class_Id` = 3187;

INSERT INTO `weenie` (`class_Id`, `class_Name`, `type`)
VALUES (3187, 'scrollbowmasteryself6', 1) /* Generic */;

INSERT INTO `weenie_properties_int` (`object_Id`, `type`, `value`)
VALUES (3187,   1,       8192) /* ItemType - Writable */
     , (3187,   2,         13) /* CreatureType - Golem */
     , (3187,   5,         30) /* EncumbranceVal */
     , (3187,  16,          8) /* ItemUseable - Contained */
     , (3187,  19,       1000) /* Value */
     , (3187,  25,         35) /* Level */
     , (3187,  44,         20) /* Damage */
     , (3187,  45,          1) /* DamageType - Slash */
     , (3187,  48,         47) /* WeaponSkill - MissileWeapons */
     , (3187,  49,         10) /* WeaponTime */
     , (3187,  65,        101) /* Placement - Resting */
     , (3187,  93,       1044) /* PhysicsState - Ethereal, IgnoreCollisions, Gravity */
     , (3187, 105,          5) /* ItemWorkmanship */
     , (3187, 131,         67) /* MaterialType - Granite */
     , (3187, 172,          5) /* AppraisalLongDescDecoration */
     , (3187, 177,          2) /* GemCount */
     , (3187, 178,         17) /* GemType */
     , (3187, 353,         10) /* WeaponType - Thrown */
     , (3187, 8030,          0);

INSERT INTO `weenie_properties_bool` (`object_Id`, `type`, `value`)
VALUES (3187,   1, False) /* Stuck */
     , (3187,  11, True ) /* IgnoreCollisions */
     , (3187,  13, True ) /* Ethereal */
     , (3187,  14, True ) /* GravityStatus */
     , (3187,  19, True ) /* Attackable */
     , (3187,  22, True ) /* Inscribable */;

INSERT INTO `weenie_properties_float` (`object_Id`, `type`, `value`)
VALUES (3187,  21,       0) /* WeaponLength */
     , (3187,  22,    0.25) /* DamageVariance */
     , (3187,  26,       0) /* MaximumVelocity */
     , (3187,  29,       1) /* WeaponDefense */
     , (3187,  39,     1.5) /* DefaultScale */
     , (3187,  62,       1) /* WeaponOffense */
     , (3187,  63,       1) /* DamageMod */;

INSERT INTO `weenie_properties_string` (`object_Id`, `type`, `value`)
VALUES (3187,   1, 'Scroll of Missile Weapon Mastery Self VI') /* Name */
     , (3187,  14, 'Use this item to attempt to learn its spell.') /* Use */
     , (3187,  16, 'Inscribed spell: Missile Weapon Mastery Self VI
Increases the caster''s Missile Weapons skill by 35 points.') /* LongDesc */;

INSERT INTO `weenie_properties_d_i_d` (`object_Id`, `type`, `value`)
VALUES (3187,   1,   33554826) /* Setup */
     , (3187,   8,  100676450) /* Icon */
     , (3187,  22,  872415275) /* PhysicsEffectTable */
     , (3187,  28,        472) /* Spell */
     , (3187, 8001,    6307864) /* PCAPRecordedWeenieHeader - Value, Usable, Container, Burden, Spell */
     , (3187, 8003,         18) /* PCAPRecordedObjectDesc - Inscribable, Attackable */
     , (3187, 8005,     135297) /* PCAPRecordedPhysicsDesc - CSetup, ObjScale, PeTable, AnimationFrame */;

INSERT INTO `weenie_properties_i_i_d` (`object_Id`, `type`, `value`)
VALUES (3187,   2, 2874282672) /* Container */
     , (3187, 8000, 2874282669) /* PCAPRecordedObjectIID */;

INSERT INTO `weenie_properties_attribute_2nd` (`object_Id`, `type`, `init_Level`, `level_From_C_P`, `c_P_Spent`, `current_Level`)
VALUES (3187,   1,  5000, 0, 0, 5000) /* MaxHealth */;

INSERT INTO `weenie_properties_spell_book` (`object_Id`, `spell`, `probability`)
VALUES (3187,   472,      2) 
     , (3187,  5344,      2) ;
