/* Weenie - MeleeWeapons - Palenqual's Okane (12114) */
DELETE FROM ace_weenie_class WHERE weenieClassId = 12114;

INSERT INTO ace_weenie_class (`weenieClassId`, `weenieClassDescription`)
VALUES (12114, 'tumerokdaggerpalenqualhi-xp');

INSERT INTO `ace_object` (`aceObjectId`, `aceObjectDescriptionFlags`, `weenieClassId`, `weenieHeaderFlags`, `weenieHeaderFlags2`, `currentMotionState`, `physicsDescriptionFlag`)
VALUES (12114, 18, 12114, 2327184, NULL, NULL, 170145);

INSERT INTO `ace_object_properties_string` (`aceObjectId`, `strPropertyId`, `propertyValue`)
VALUES (12114, 1, 'Palenqual''s Okane') /* NAME_STRING */;

INSERT INTO `ace_object_properties_did` (`aceObjectId`, `didPropertyId`, `propertyValue`)
VALUES (12114, 8, 100672032) /* ICON_DID */
     , (12114, 1, 33557234) /* SETUP_DID */
     , (12114, 3, 536870932) /* SOUND_TABLE_DID */
     , (12114, 22, 872415275) /* PHYSICS_EFFECT_TABLE_DID */;

INSERT INTO `ace_object_properties_int` (`aceObjectId`, `intPropertyId`, `propertyValue`)
VALUES (12114, 53, 1) /* PLACEMENT_POSITION_INT */
     , (12114, 1, 1) /* ITEM_TYPE_INT */
     , (12114, 5, 135) /* ENCUMB_VAL_INT */
     , (12114, 51, 1) /* COMBAT_USE_INT */
     , (12114, 18, 1) /* UI_EFFECTS_INT */
     , (12114, 16, 1) /* ITEM_USEABLE_INT */
     , (12114, 9, 1048576) /* LOCATIONS_INT */
     , (12114, 52, 1) /* PARENT_LOCATION_INT */
     , (12114, 93, 1044) /* PHYSICS_STATE_INT */
     , (12114, 9007, 6) /* MeleeWeapon_WeenieType */;

INSERT INTO `ace_object_properties_double` (`aceObjectId`, `dblPropertyId`, `propertyValue`)
VALUES (12114, 39, 1.2) /* DEFAULT_SCALE_FLOAT */;

INSERT INTO `ace_object_properties_bool` (`aceObjectId`, `boolPropertyId`, `propertyValue`)
VALUES (12114, 13, True) /* ETHEREAL_BOOL */
     , (12114, 11, True) /* IGNORE_COLLISIONS_BOOL */
     , (12114, 14, True) /* GRAVITY_STATUS_BOOL */
     , (12114, 19, True) /* ATTACKABLE_BOOL */
     , (12114, 22, True) /* INSCRIBABLE_BOOL */;

