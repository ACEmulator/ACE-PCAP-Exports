/* Weenie - MiscObjects - Enduring Enchantment (29271) */
DELETE FROM ace_weenie_class WHERE weenieClassId = 29271;

INSERT INTO ace_weenie_class (`weenieClassId`, `weenieClassDescription`)
VALUES (29271, 'gemaugmentationdeathspellsremain');

INSERT INTO `ace_object` (`aceObjectId`, `aceObjectDescriptionFlags`, `weenieClassId`, `weenieHeaderFlags`, `weenieHeaderFlags2`, `currentMotionState`, `physicsDescriptionFlag`)
VALUES (29271, 18, 29271, 2113552, NULL, NULL, 137217);

INSERT INTO `ace_object_properties_string` (`aceObjectId`, `strPropertyId`, `propertyValue`)
VALUES (29271, 1, 'Enduring Enchantment') /* NAME_STRING */;

INSERT INTO `ace_object_properties_did` (`aceObjectId`, `didPropertyId`, `propertyValue`)
VALUES (29271, 8, 100686474) /* ICON_DID */
     , (29271, 1, 33554809) /* SETUP_DID */
     , (29271, 3, 536870932) /* SOUND_TABLE_DID */
     , (29271, 22, 872415275) /* PHYSICS_EFFECT_TABLE_DID */;

INSERT INTO `ace_object_properties_int` (`aceObjectId`, `intPropertyId`, `propertyValue`)
VALUES (29271, 53, 101) /* PLACEMENT_POSITION_INT */
     , (29271, 1, 128) /* ITEM_TYPE_INT */
     , (29271, 5, 50) /* ENCUMB_VAL_INT */
     , (29271, 16, 8) /* ITEM_USEABLE_INT */
     , (29271, 93, 1044) /* PHYSICS_STATE_INT */
     , (29271, 9007, 1) /* Generic_WeenieType */;

INSERT INTO `ace_object_properties_bool` (`aceObjectId`, `boolPropertyId`, `propertyValue`)
VALUES (29271, 13, True) /* ETHEREAL_BOOL */
     , (29271, 11, True) /* IGNORE_COLLISIONS_BOOL */
     , (29271, 14, True) /* GRAVITY_STATUS_BOOL */
     , (29271, 19, True) /* ATTACKABLE_BOOL */
     , (29271, 22, True) /* INSCRIBABLE_BOOL */;

/* Extended Appraisal Data */

REPLACE INTO `ace_object_properties_string` (`aceObjectId`, `strPropertyId`, `propertyValue`)
VALUES (29271, 16, 'After using this gem, spells that were cast on you prior to death will remain in effect when you resurrect at your Lifestone. Spells will still expire on death if you are killed in a PK battle. Spells will remain if you are killed in PKLite. This augmentation cannot be repeated.') /* LONG_DESC_STRING */;

REPLACE INTO `ace_object_properties_int` (`aceObjectId`, `intPropertyId`, `propertyValue`)
VALUES (29271, 33, 1) /* BONDED_INT */
     , (29271, 114, 1) /* ATTUNED_INT */
     , (29271, 19, 0) /* VALUE_INT */
     , (29271, 5, 50) /* ENCUMB_VAL_INT */;

REPLACE INTO `ace_object_properties_bigint` (`aceObjectId`, `bigIntPropertyId`, `propertyValue`)
VALUES (29271, 3, 4000000000) /* AUGMENTATION_COST_INT64 */;

