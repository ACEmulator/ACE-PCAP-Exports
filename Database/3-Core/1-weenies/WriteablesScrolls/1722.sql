/* Weenie - WriteablesScrolls - Scroll of Magic Item Tinkering Expertise Other (1722) */
DELETE FROM ace_weenie_class WHERE weenieClassId = 1722;

INSERT INTO ace_weenie_class (`weenieClassId`, `weenieClassDescription`)
VALUES (1722, 'scrollmagicitemexpertiseother');

INSERT INTO `ace_object` (`aceObjectId`, `aceObjectDescriptionFlags`, `weenieClassId`, `weenieHeaderFlags`, `weenieHeaderFlags2`, `currentMotionState`, `physicsDescriptionFlag`)
VALUES (1722, 18, 1722, 6307864, NULL, NULL, 135297);

INSERT INTO `ace_object_properties_string` (`aceObjectId`, `strPropertyId`, `propertyValue`)
VALUES (1722, 1, 'Scroll of Magic Item Tinkering Expertise Other') /* NAME_STRING */;

INSERT INTO `ace_object_properties_did` (`aceObjectId`, `didPropertyId`, `propertyValue`)
VALUES (1722, 8, 100676477) /* ICON_DID */
     , (1722, 1, 33554826) /* SETUP_DID */
     , (1722, 22, 872415275) /* PHYSICS_EFFECT_TABLE_DID */
     , (1722, 28, 756) /* SPELL_DID - MagicItemExpertiseOther1_SpellID */;

INSERT INTO `ace_object_properties_int` (`aceObjectId`, `intPropertyId`, `propertyValue`)
VALUES (1722, 53, 101) /* PLACEMENT_POSITION_INT */
     , (1722, 1, 8192) /* ITEM_TYPE_INT */
     , (1722, 5, 30) /* ENCUMB_VAL_INT */
     , (1722, 16, 8) /* ITEM_USEABLE_INT */
     , (1722, 19, 1) /* VALUE_INT */
     , (1722, 93, 1044) /* PHYSICS_STATE_INT */
     , (1722, 9007, 34) /* Scroll_WeenieType */;

INSERT INTO `ace_object_properties_double` (`aceObjectId`, `dblPropertyId`, `propertyValue`)
VALUES (1722, 39, 1.5) /* DEFAULT_SCALE_FLOAT */;

INSERT INTO `ace_object_properties_bool` (`aceObjectId`, `boolPropertyId`, `propertyValue`)
VALUES (1722, 13, True) /* ETHEREAL_BOOL */
     , (1722, 11, True) /* IGNORE_COLLISIONS_BOOL */
     , (1722, 14, True) /* GRAVITY_STATUS_BOOL */
     , (1722, 19, True) /* ATTACKABLE_BOOL */
     , (1722, 22, True) /* INSCRIBABLE_BOOL */;

