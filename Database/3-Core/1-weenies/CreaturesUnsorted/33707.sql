/* Weenie - CreaturesUnsorted - Depraved Shadow (33707) */
DELETE FROM ace_weenie_class WHERE weenieClassId = 33707;

INSERT INTO ace_weenie_class (`weenieClassId`, `weenieClassDescription`)
VALUES (33707, 'ace33707-depravedshadow');

INSERT INTO `ace_object` (`aceObjectId`, `aceObjectDescriptionFlags`, `weenieClassId`, `weenieHeaderFlags`, `weenieHeaderFlags2`, `currentMotionState`, `physicsDescriptionFlag`)
VALUES (33707, 20, 33707, 8388630, NULL, 'AAA9AAAAAAA=', 104643);

INSERT INTO `ace_object_properties_string` (`aceObjectId`, `strPropertyId`, `propertyValue`)
VALUES (33707, 1, 'Depraved Shadow') /* NAME_STRING */;

INSERT INTO `ace_object_properties_did` (`aceObjectId`, `didPropertyId`, `propertyValue`)
VALUES (33707, 8, 100670398) /* ICON_DID */
     , (33707, 1, 33554510) /* SETUP_DID */
     , (33707, 3, 536870914) /* SOUND_TABLE_DID */
     , (33707, 2, 150994945) /* MOTION_TABLE_DID */
     , (33707, 22, 872415331) /* PHYSICS_EFFECT_TABLE_DID */
     , (33707, 6, 67108990) /* PALETTE_BASE_DID */;

INSERT INTO `ace_object_properties_int` (`aceObjectId`, `intPropertyId`, `propertyValue`)
VALUES (33707, 1, 16) /* ITEM_TYPE_INT */
     , (33707, 7, 255) /* CONTAINERS_CAPACITY_INT */
     , (33707, 6, 255) /* ITEMS_CAPACITY_INT */
     , (33707, 133, 2) /* SHOWABLE_ON_RADAR_INT */
     , (33707, 16, 1) /* ITEM_USEABLE_INT */
     , (33707, 93, 4195336) /* PHYSICS_STATE_INT */
     , (33707, 9007, 10) /* Creature_WeenieType */;

INSERT INTO `ace_object_properties_double` (`aceObjectId`, `dblPropertyId`, `propertyValue`)
VALUES (33707, 39, 1.1) /* DEFAULT_SCALE_FLOAT */;

INSERT INTO `ace_object_properties_bool` (`aceObjectId`, `boolPropertyId`, `propertyValue`)
VALUES (33707, 12, True) /* REPORT_COLLISIONS_BOOL */
     , (33707, 14, True) /* GRAVITY_STATUS_BOOL */
     , (33707, 42, True) /* ALLOW_EDGE_SLIDE_BOOL */
     , (33707, 19, True) /* ATTACKABLE_BOOL */
     , (33707, 1, True) /* STUCK_BOOL */;

INSERT INTO `ace_object_palette_change` (`aceObjectId`, `subPaletteId`, `offset`, `length`)
VALUES (33707, 67109561, 0, 24)
     , (33707, 67116990, 24, 8)
     , (33707, 67109565, 32, 8)
     , (33707, 67112917, 64, 8)
     , (33707, 67110026, 72, 8)
     , (33707, 67112917, 40, 24)
     , (33707, 67109969, 92, 4)
     , (33707, 67113926, 136, 16)
     , (33707, 67113926, 174, 66)
     , (33707, 67113926, 80, 12)
     , (33707, 67113926, 116, 12)
     , (33707, 67113926, 96, 12)
     , (33707, 67113926, 168, 6)
     , (33707, 67113926, 160, 8);

INSERT INTO `ace_object_texture_map_change` (`aceObjectId`, `index`, `oldId`, `newId`)
VALUES (33707, 16, 83886232, 83890685)
     , (33707, 16, 83886668, 83890284)
     , (33707, 16, 83886837, 83890294)
     , (33707, 16, 83886684, 83890346)
     , (33707, 5, 83887064, 83886241)
     , (33707, 1, 83887064, 83886241)
     , (33707, 6, 83887066, 83887055)
     , (33707, 2, 83887066, 83887055)
     , (33707, 9, 83887070, 83886781)
     , (33707, 9, 83887062, 83886686)
     , (33707, 0, 83889072, 83886685)
     , (33707, 0, 83889342, 83889386)
     , (33707, 10, 83887069, 83886782)
     , (33707, 13, 83887069, 83886782)
     , (33707, 11, 83887067, 83891213)
     , (33707, 14, 83887067, 83891213)
     , (33707, 5, 83894182, 83894182)
     , (33707, 1, 83894182, 83894182)
     , (33707, 6, 83894182, 83894182)
     , (33707, 2, 83894182, 83894182)
     , (33707, 9, 83894176, 83894176)
     , (33707, 9, 83894178, 83894178)
     , (33707, 0, 83894171, 83894171)
     , (33707, 13, 83894173, 83894173)
     , (33707, 13, 83894175, 83894175)
     , (33707, 10, 83894174, 83894174)
     , (33707, 14, 83894172, 83894172)
     , (33707, 14, 83894185, 83894185)
     , (33707, 11, 83894172, 83894172)
     , (33707, 15, 83894179, 83894179)
     , (33707, 12, 83894179, 83894179)
     , (33707, 3, 83894184, 83894184)
     , (33707, 7, 83894184, 83894184)
     , (33707, 4, 83894184, 83894184)
     , (33707, 8, 83894184, 83894184);

INSERT INTO `ace_object_animation_change` (`aceObjectId`, `index`, `animationId`)
VALUES (33707, 5, 16788087)
     , (33707, 1, 16788083)
     , (33707, 6, 16788086)
     , (33707, 2, 16788085)
     , (33707, 9, 16788080)
     , (33707, 0, 16788097)
     , (33707, 13, 16788099)
     , (33707, 10, 16788090)
     , (33707, 14, 16788092)
     , (33707, 11, 16788084)
     , (33707, 15, 16788095)
     , (33707, 12, 16788094)
     , (33707, 3, 16788081)
     , (33707, 7, 16788082)
     , (33707, 4, 16788088)
     , (33707, 8, 16788089)
     , (33707, 16, 16792966);

/* Extended Apprasial Data */

REPLACE INTO `ace_object_properties_did` (`aceObjectId`, `didPropertyId`, `propertyValue`)
VALUES (33707, 16, 67109565) /* EYES_PALETTE_DID */
     , (33707, 9, 83890284) /* EYES_TEXTURE_DID */
     , (33707, 17, 67109561) /* SKIN_PALETTE_DID */
     , (33707, 10, 83890294) /* NOSE_TEXTURE_DID */
     , (33707, 11, 83890346) /* MOUTH_TEXTURE_DID */
     , (33707, 15, 67116990) /* HAIR_PALETTE_DID */;

REPLACE INTO `ace_object_properties_int` (`aceObjectId`, `intPropertyId`, `propertyValue`)
VALUES (33707, 113, 2) /* GENDER_INT */
     , (33707, 2, 22) /* CREATURE_TYPE_INT */
     , (33707, 25, 200) /* LEVEL_INT */
     , (33707, 188, 1) /* HERITAGE_GROUP_INT */;

REPLACE INTO `ace_object_properties_attribute2nd` (`aceObjectId`, `attribute2ndId`, `attribute2ndValue`)
VALUES (33707, 64, 1000) /* MAX_HEALTH_ATTRIBUTE_2ND */;

