INSERT INTO ace_weenie_class (`weenieClassId`, `weenieClassDescription`)
VALUES (43481, 'ace43481-yougonnadie');

INSERT INTO `ace_object` (`aceObjectId`, `aceObjectDescriptionFlags`, `weenieClassId`, `weenieHeaderFlags`, `currentMotionState`, `physicsDescriptionFlag`)
VALUES (43481, 8388630, 43481, 60, 'AAA9AAAAAAA=', 104579);

INSERT INTO `ace_object_properties_string` (`aceObjectId`, `strPropertyId`, `propertyValue`)
VALUES (43481, 1, 'Yougonnadie');

INSERT INTO `ace_object_properties_did` (`aceObjectId`, `didPropertyId`, `propertyValue`)
VALUES (43481, 8, 100674626)
     , (43481, 1, 33561120)
     , (43481, 3, 536871070)
     , (43481, 2, 150995461)
     , (43481, 22, 872415398)
     , (43481, 6, 67116974);

INSERT INTO `ace_object_properties_int` (`aceObjectId`, `intPropertyId`, `propertyValue`)
VALUES (43481, 1, 16)
     , (43481, 7, 7)
     , (43481, 6, 102)
     , (43481, 133, 4)
     , (43481, 16, 1)
     , (43481, 93, 4211728);

INSERT INTO `ace_object_properties_double` (`aceObjectId`, `dblPropertyId`, `propertyValue`)
VALUES (43481, 39, 0.6);

INSERT INTO `ace_object_properties_bool` (`aceObjectId`, `boolPropertyId`, `propertyValue`)
VALUES (43481, 11, True)
     , (43481, 14, True)
     , (43481, 18, True)
     , (43481, 42, True);

INSERT INTO `ace_object_palette_change` (`aceObjectId`, `subPaletteId`, `offset`, `length`)
VALUES (43481, 67116914, 0, 24)
     , (43481, 67116898, 24, 8)
     , (43481, 67116854, 32, 8);

INSERT INTO `ace_object_texture_map_change` (`aceObjectId`, `index`, `oldId`, `newId`)
VALUES (43481, 0, 83891927, 83891927);

INSERT INTO ace_weenie_class (`weenieClassId`, `weenieClassDescription`)
VALUES (43480, 'ace43480-iamolthoi');

INSERT INTO `ace_object` (`aceObjectId`, `aceObjectDescriptionFlags`, `weenieClassId`, `weenieHeaderFlags`, `currentMotionState`, `physicsDescriptionFlag`)
VALUES (43480, 8388630, 43480, 60, 'AAA8AAEAAAA8AAAA', 104579);

INSERT INTO `ace_object_properties_string` (`aceObjectId`, `strPropertyId`, `propertyValue`)
VALUES (43480, 1, 'Iamolthoi');

INSERT INTO `ace_object_properties_did` (`aceObjectId`, `didPropertyId`, `propertyValue`)
VALUES (43480, 8, 100667623)
     , (43480, 1, 33561121)
     , (43480, 3, 536870925)
     , (43480, 2, 150995462)
     , (43480, 22, 872415265)
     , (43480, 6, 67116973);

INSERT INTO `ace_object_properties_int` (`aceObjectId`, `intPropertyId`, `propertyValue`)
VALUES (43480, 1, 16)
     , (43480, 7, 7)
     , (43480, 6, 102)
     , (43480, 133, 4)
     , (43480, 16, 1)
     , (43480, 93, 4195336);

INSERT INTO `ace_object_properties_double` (`aceObjectId`, `dblPropertyId`, `propertyValue`)
VALUES (43480, 39, 0.9);

INSERT INTO `ace_object_properties_bool` (`aceObjectId`, `boolPropertyId`, `propertyValue`)
VALUES (43480, 12, True)
     , (43480, 14, True)
     , (43480, 42, True);

INSERT INTO `ace_object_palette_change` (`aceObjectId`, `subPaletteId`, `offset`, `length`)
VALUES (43480, 67116908, 0, 24)
     , (43480, 67116892, 24, 8)
     , (43480, 67116855, 32, 8);

INSERT INTO `ace_object_texture_map_change` (`aceObjectId`, `index`, `oldId`, `newId`)
VALUES (43480, 0, 83898333, 83898333)
     , (43480, 1, 83898335, 83898335)
     , (43480, 2, 83898332, 83898332)
     , (43480, 2, 83898334, 83898334)
     , (43480, 3, 83898332, 83898332)
     , (43480, 4, 83898332, 83898332)
     , (43480, 4, 83898334, 83898334)
     , (43480, 5, 83898332, 83898332)
     , (43480, 6, 83898332, 83898332)
     , (43480, 7, 83898332, 83898332)
     , (43480, 8, 83898332, 83898332)
     , (43480, 9, 83898332, 83898332)
     , (43480, 10, 83898332, 83898332)
     , (43480, 11, 83898332, 83898332)
     , (43480, 12, 83898332, 83898332)
     , (43480, 13, 83898332, 83898332)
     , (43480, 14, 83898333, 83898333)
     , (43480, 15, 83898336, 83898336)
     , (43480, 16, 83898336, 83898336)
     , (43480, 16, 83898334, 83898334)
     , (43480, 17, 83898336, 83898336)
     , (43480, 18, 83898336, 83898336)
     , (43480, 19, 83898336, 83898336)
     , (43480, 20, 83898336, 83898336)
     , (43480, 20, 83898334, 83898334)
     , (43480, 21, 83898336, 83898336)
     , (43480, 22, 83898336, 83898336)
     , (43480, 23, 83898332, 83898332)
     , (43480, 24, 83898332, 83898332)
     , (43480, 1, 83891927, 83891927);

